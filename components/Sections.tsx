...
function Counter({ to }: { to: number }) {
  ...
  return <p ref={ref} className="text-4xl font-semibold text-white">{count}</p>;
}
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-26 px-4 pb-24 sm:px-6 lg:px-8">
      <MotionReveal>
        <section id="courses" className="space-y-7">
          <h2 className="font-display text-3xl text-white sm:text-4xl">Courses designed for measurable growth</h2>
          <div className="grid gap-4 md:grid-cols-2">
            {courses.map((course, idx) => (
              ...
              >
                ...
                <p className="mt-2 text-zinc-300">{course.desc}</p>
                <span className="mt-4 inline-block text-sm text-emerald-200">Explore →</span>
              </motion.article>
            ))}
          </div>
        </section>
      ...
      <MotionReveal>
        <section id="results" className="space-y-7">
          <h2 className="font-display text-3xl text-white sm:text-4xl">Results & achievements</h2>
          <div className="grid gap-4 md:grid-cols-3">
            {achievements.map((item) => (
              <article key={item.label} className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-center">
                <Counter to={item.value} />
                <p className="mt-3 text-sm tracking-[0.2em] text-zinc-400 uppercase">{item.label}</p>
              </article>
            ))}
          </div>
        </section>
      ...
      <MotionReveal>
        <section id="testimonials" className="space-y-7">
          <h2 className="font-display text-3xl text-white sm:text-4xl">What students say</h2>
          <div className="grid gap-4 md:grid-cols-2">
            {testimonials.map(([name, quote]) => (
              ...
              >
                ...
                <footer className="mt-4 text-sm text-emerald-200">— {name}</footer>
              </motion.blockquote>
            ))}
          </div>
        </section>
      ...
      <MotionReveal>
        <section className="space-y-7">
          <h2 className="font-display text-3xl text-white sm:text-4xl">Why choose BrightPath</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((feature) => (
              <div key={feature} className="rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4">
                <p className="text-zinc-100">✦ {feature}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <MotionReveal>
        <section id="location" className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
          ...
          <article className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
            <h3 className="text-xl text-white">Location & Timings</h3>
            <p className="mt-4 text-zinc-300">SCO 21, Sector 70, Mohali, Punjab</p>
            <p className="mt-2 text-zinc-300">Mon - Sat: 8:00 AM - 8:00 PM</p>
            <p className="mt-2 text-zinc-300">Sunday: By appointment</p>
            <p className="mt-4 text-emerald-200">Call: +91 98765 43210</p>
          </article>
        </section>
      </MotionReveal>

      <MotionReveal>
        <section id="contact" className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 sm:p-9">
          <h2 className="font-display text-3xl text-white sm:text-4xl">Let&apos;s plan your learning path</h2>
          <p className="mt-3 text-zinc-300">Fill the details and we&apos;ll open WhatsApp with your message pre-filled.</p>
          <form onSubmit={onSubmit} className="mt-7 grid gap-4 md:grid-cols-2">
            ...
            <button className="rounded-xl bg-emerald-300 px-5 py-3 font-semibold text-[#042019] md:col-span-2" type="submit">
              Send on WhatsApp
            </button>
          </form>
        </section>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 12 }}
            className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full border border-emerald-200/30 bg-emerald-300 px-4 py-2 text-sm font-medium text-[#042019]"
          >
